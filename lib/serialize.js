...
export default async function serialize(conn, msg) {
  ...
  if (msg.key) {
    ...
    m.sender = conn.getJid(
      jidNormalizedUser(m.key.participantAlt || m.key.participantPn || m.key.participant || m.chat)
    );
  ...
  if (m.message) {
    ...
    if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
        ...
        m.quoted.msg =
          parseMessage(m.quoted.message[m.quoted.type]) || m.quoted.message[m.quoted.type];
        m.quoted.isMedia =
        ...
        m.quoted.sender = conn.getJid(
          jidNormalizedUser(m.msg?.contextInfo?.participant || m.quoted.from)
        );
